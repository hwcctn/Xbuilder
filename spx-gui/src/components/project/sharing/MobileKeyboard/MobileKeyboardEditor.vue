...
const allPool = ref<string[]>(webKeys.map((k) => props.projectKeys?.includes(k) ? null : k).filter((k) => k !== null))
const autoPool = ref<string[]>([])
if (props.projectKeys) {
  autoPool.value = props.projectKeys
}
